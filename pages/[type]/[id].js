...
export default function WatchPage() {
  ...
  const generateShareImage = useCallback(async () => {
    ...
    // Carregar e desenhar o backdrop
    ...
    const drawOverlay = () => {
      ...
      logoImg.onload = () => {
        const logoSize = 80
        ctx.drawImage(logoImg, canvas.width - logoSize - 40, canvas.height - logoSize - 20, logoSize, logoSize)
        ...
        // Subtítulo "Assistir no Yoshikawa Streaming"
        ctx.font = '28px Inter, sans-serif'
        ctx.fillStyle = '#cccccc'
        ctx.shadowBlur = 6
        ctx.fillText('Assistir no Yoshikawa Streaming', 60, y + 10)
      ...
      <Head>
        ...
        <style>{`
          *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}
          ...
          .share-modal-overlay{position:fixed;inset:0;z-index:3000;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;padding:20px}
          ...
          .share-modal img{max-width:100%;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.5)}
      ...
      {showShareModal && (
        <div className="share-modal-overlay" onClick={() => setShowShareModal(false)}>
          <div className="share-modal" onClick={e => e.stopPropagation()}>
            ...
            {shareImageLoading ? (
              <div style={{ width: '100%', aspectRatio: '1200/630', background: '#2a2a2a', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
