...
export default function WatchPage() {
  ...
  const showToast = (msg, type = 'info') => {
    if (currentToast && !currentToast.closing) {
      ...
      setTimeout(() => setToastQueue(prev => [...prev, { message: msg, type, id: Date.now() }]), 300)
    } else setToastQueue(prev => [...prev, { message: msg, type, id: Date.now() }])
  }
  ...
  useEffect(() => {
    if (toastQueue.length > 0) {
      if (currentToast && !currentToast.closing) {
        ...
        toastTimerRef.current = setTimeout(() => setCurrentToast(t => t?.id === next.id ? { ...t, closing: true } : t), 2500)
      }
    }
  ...
  return (
    <>
      <Head>
        ...
        <style>{`
          * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
          body { font-family: 'Inter', sans-serif; background: #050505; color: #fff; line-height: 1.6; overflow-x: hidden; }
          ...
          .glass-panel { background: rgba(255,255,255,0.06); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.15); border-radius: inherit; }
          ...
          .continue-btn { display: flex; align-items: center; gap: 8px; padding: 10px 24px; background: #F05454; border-radius: 28px; color: #fff; font-weight: 700; font-size: clamp(14px,2vw,16px); cursor: pointer; border: none; width: fit-content; }
          ...
          .social-item { display: flex; flex-direction: column; align-items: center; gap: 4px; color: rgba(255,255,255,0.7); cursor: pointer; font-size: 13px; }
          .social-item i { font-size: 22px; }
          .social-item.active i { color: #FF5B5B; }

          .synopsis { padding: 0 16px 20px; }
          .synopsis p { font-size: 15px; line-height: 1.45; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; }
          .synopsis p.expanded { -webkit-line-clamp: unset; }
          .synopsis-toggle { display: flex; justify-content: center; margin-top: 12px; color: rgba(255,255,255,0.6); cursor: pointer; font-size: 13px; }
          ...
          .episodes-list { padding: 0 12px 100px; display: flex; flex-direction: column; gap: 10px; }
          .ep-card { display: flex; gap: 12px; background: #121212; border-radius: 16px; padding: 12px; cursor: pointer; transition: background 0.2s; }
          .ep-card:hover { background: #1a1a1a; }
          .ep-card.active { border: 1px solid rgba(255,255,255,0.2); }
          .ep-thumb { width: 140px; height: 80px; border-radius: 12px; overflow: hidden; background: #2a2a2a; flex-shrink: 0; }
          ...
          .ep-watched { color: #4CAF50; margin-left: auto; align-self: center; font-size: 16px; }

          .player-overlay { position: fixed; inset: 0; z-index: 2000; background: rgba(0,0,0,0.8); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); display: flex; align-items: center; justify-content: center; padding: 16px; }
          ...
          .toast-wrap { position: fixed; top: 80px; left: 50%; z-index: 3000; transform: translateX(-50%); }
          .toast { display: flex; align-items: center; gap: 10px; padding: 12px 16px; border-radius: 22px; background: #1B1B1B; box-shadow: 0 10px 30px rgba(0,0,0,0.5); min-width: 250px; }
          .toast-icon-wrapper { width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
          .toast.success .toast-icon-wrapper { background: #4CAF50; } .toast.info .toast-icon-wrapper { background: #2196F3; } .toast.error .toast-icon-wrapper { background: #f44336; }
          .toast-title { font-weight: 600; font-size: 14px; } .toast-msg { font-size: 12px; color: rgba(255,255,255,0.7); }
      ...
      {content && (
        <>
          <div className="hero">
            <img src={content.backdrop_path ? `https://image.tmdb.org/t/p/original${content.backdrop_path}` : DEFAULT_BACKDROP} alt="" />
          ...
          <div className="social-bar">
            <div className="social-item"><i className="fas fa-comment"></i><span>Comentários</span></div>
            <div className="social-item"><i className="fas fa-thumbs-up"></i><span>{Math.round(content.popularity || 0)}</span></div>
            <div className={`social-item ${isFavorite ? 'active' : ''}`} onClick={toggleFavorite}>
              <i className={isFavorite ? 'fas fa-heart' : 'far fa-heart'}></i><span>{isFavorite ? 'Favoritado' : 'Favoritar'}</span>
            </div>
            <div className="social-item" onClick={() => navigator.share?.({ title: content.title || content.name, url: window.location.href })}>
              <i className="fas fa-share-alt"></i><span>Compartilhar</span>
            </div>
          </div>
          ...
          <div className="synopsis">
            ...
            {content.overview?.length > 200 && (
              <div className="synopsis-toggle" onClick={() => setSynopsisExpanded(!synopsisExpanded)}>
            ...
            <>
              ...
              <div className="episodes-list">
                {orderedEps.map(ep => {
                  ...
                  return (
                    <div key={ep.id} className={`ep-card ${ep.episode_number === episode ? 'active' : ''}`} onClick={() => handleEpisodeClick(ep.episode_number)}>
                      <div className="ep-thumb">
                        {ep.still_path ? <img src={`https://image.tmdb.org/t/p/w300${ep.still_path}`} /> : <div style={{ color: '#444', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}><i className="fas fa-image"></i></div>}
                      </div>
                      <div className="ep-info">
                        <h4>{ep.episode_number}. {ep.name || 'Sem título'}</h4>
                        <span>{ep.runtime ? `${ep.runtime} min` : 'Duração indisponível'}</span>
                      </div>
                      {watched && <div className="ep-watched"><i className="fas fa-check-circle"></i></div>}
                    </div>
                  )
                })}
              </div>
          ...
          <div className="player-box">
            ...
            {type === 'tv' && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 8 }}>
                <button className="continue-btn" onClick={() => setEpisode(e => Math.max(1, e - 1))} disabled={episode === 1}><i className="fas fa-backward"></i> Anterior</button>
                <button className="continue-btn" onClick={() => {
                  if (seasonData && episode < seasonData.episodes.length) setEpisode(e => e + 1)
                }}>Próximo <i className="fas fa-forward"></i></button>
              </div>
            )}
          </div>
        </div>
      ...
      <ToastContainer toast={currentToast} closeToast={() => setCurrentToast(t => t ? { ...t, closing: true } : null)} />
    </>
  )
  }
