...
export default function WatchPage() {
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'room_users', filter: `room_id=eq.${roomId}` }, (payload) => {
        setRoomUsers(prev => {
          ...
          const systemMsg = {
            ...
            user_name: 'Sistema',
            user_avatar: '',
            content: `${payload.new.user_name} entrou na sala`,
            is_system: true,
      ...
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'room_users', filter: `room_id=eq.${roomId}` }, (payload) => {
        setRoomUsers(prev => {
          const systemMsg = {
            ...
            user_name: 'Sistema',
            user_avatar: '',
            content: `${payload.old.user_name} saiu da sala`,
            is_system: true,
            created_at: new Date().toISOString()
          }
          setMessages(prev => [...prev, systemMsg])
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'rooms', filter: `id=eq.${roomId}` }, (payload) => {
        if (payload.new.is_active === false) {
          const systemMsg = {
            ...
            user_name: 'Sistema',
            user_avatar: '',
            content: 'A sala foi fechada pelo criador',
            is_system: true,
            created_at: new Date().toISOString()
          }
          setMessages(prev => [...prev, systemMsg])
  ...
  return (
    <>
      ...
      {isPlaying && (
        <div className="player-overlay" style={{ background: bgBlack ? '#101010' : 'rgba(0,0,0,0.1)', backdropFilter: bgBlack ? 'none' : 'blur(40px)', WebkitBackdropFilter: bgBlack ? 'none' : 'blur(40px)' }}>
              ...
              <div className="player-controls">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    ...
                    title={bgBlack ? 'Fundo com blur' : 'Fundo preto absoluto'}
                  >
