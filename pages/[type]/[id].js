...
import { useRouter } from 'next/router'const TMDB_API_KEY = '66223dd3ad2885cf1129b181c7826287'
const DEFAULT_BACKDROP = 'https://yoshikawa-bot.github.io/cache/images/14c34900.jpg'export const Header = ({ 
  label, scrolled, 
  ...
  navHidden
    ...
    <>
      <header className={bar-container top-bar ${scrolled ? 'scrolled-state' : ''} ${navHidden ? 'nav-hidden' : ''}}>    <button 
  ...
  const [animating, setAnimating] = useState(false)  const handleShare = async () => {
    if (navigator.share) {
    ...
    <div className={bar-container bottom-bar ${navHidden ? 'nav-hidden' : ''}}>
      ...
      </button>  <div className={`pill-container glass-panel ${navHidden ? 'hidden-pill' : ''}`}>
  ...
  const carouselRef = useRef(null)  const [isLoading, setIsLoading] = useState(true)
  const [navHidden, setNavHidden] = useState(false)  const [scrolled, setScrolled] = useState(false)
  ...
  const [dataClosing, setDataClosing] = useState(false)  const [currentToast, setCurrentToast] = useState(null)
  const [toastQueue, setToastQueue] = useState([])  const [content, setContent] = useState(null)
  const [isPlaying, setIsPlaying] = useState(false)
  const [isWideMode, setIsWideMode] = useState(false)
  const [isFavorite, setIsFavorite] = useState(false)  const [season, setSeason] = useState(1)
  const [episode, setEpisode] = useState(1)
  const [seasonData, setSeasonData] = useState(null)
  const [expandedDetails, setExpandedDetails] = useState(false)  const toastTimerRef = useRef(null)  useEffect(() => {
    if (content) {
...
loadContent()  }, [id, type])  const fetchSeason = async (tvId, seasonNum) => {
    try {
  ...
  showToast('Erro ao salvar favorito', 'error')
  ...
  }, [showInfoPopup, infoClosing, showTechPopup, techClosing, showSynopsisPopup, synopsisClosing, showDataPopup, dataClosing, currentToast])  const toggleInfoPopup = () => {
    ...
    window.addEventListener('scroll', onScroll, { passive: true })const onClick = (e) => { 
...
return () => { 
  ...
  }, [episode, seasonData])  const handleNextEp = () => {
  ...
  const currentEpisodeData = seasonData?.episodes?.find(e => e.episode_number === episode)  return (
    <>
      <Head>
      ...
      .bottom-bar.nav-hidden .nav-btn:not(.hide-toggle-pill-btn) {
        ...
        overflow: hidden;
        pointer-events: none;
        transition: opacity 0.6s var(--ease-smooth), width 0.6s var(--ease-smooth);
      }
      ...
      <main className="container">
        ...
        <div className="glass-panel details-container">
          <div className="text-left">
            ...
            {type === 'tv' && (
              <button className="expand-btn" onClick={() => setExpandedDetails(!expandedDetails)}>
                <i className={expandedDetails ? "fas fa-chevron-up" : "fas fa-chevron-down"}></i>
                {expandedDetails ? "Ocultar Detalhes" : "Expandir Detalhes"}
              </button>
            )}
          </div>

          {type === 'tv' && expandedDetails && (
            <>
